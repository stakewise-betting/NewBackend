...
const buildCommentTree = (comments) => {
    ...
    comments.forEach(comment => {
        comment = comment.toObject(); // Convert Mongoose doc to plain object to add properties
        comment.replies = [];
    ...
    comments.forEach(commentData => {
        const comment = commentMap[commentData._id.toString()]; // Get the object from map
        if (comment.parentId) {
    ...
    try {
        ...
        const processedComments = allComments.map(comment => {
            if (comment.isDeleted) {
                return {
                    ...comment.toObject(), // Keep essential fields like _id, parentId, createdAt, replies array placeholder
                    text: "[deleted]",
            ...
            // comment.username = comment.userId ? `${comment.userId.fname} ${comment.userId.lname}` : 'User';
            return comment; // Return the original comment if not deleted
        });
